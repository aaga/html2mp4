...
parser = argparse.ArgumentParser(description="Render HTML files to MP4 movies with subtitles")
parser.add_argument("filename", help="Path to HTML file")
parser.add_argument("--theme", help="Choose from BASIC, BASIC_WITH_SOUND, or STAR_WARS. Defaults to BASIC")
parser.add_argument("--darkmode", action="store_true", help="Whether to enable dark mode (only works for Star Wars theme)")
parser.set_defaults(theme="BASIC")
parser.set_defaults(darkmode=False)
...
def main():
    ...
    with open(args.filename) as fp:
        ...
        # Recursively parse body into a Shot List
        layout(body)
        ...
        print("Succesfully parsed HTML file to a Shot List:")
        shot_list.debug_print()
        print("Proceeding to render...")

        if (args.theme == "BASIC"):
            print("Rendering basic movie...")
            make_basic_movie(shot_list, with_tts=False)
        elif (args.theme == "BASIC_WITH_SOUND"):
            print("Rendering basic movie with sound...")
            make_basic_movie(shot_list, with_tts=True)
        elif (args.theme == "STAR_WARS"):
            print("Rendering Star Wars movie", "with Dark Mode enabled" if args.darkmode else "", "...")
            make_star_wars_movie(shot_list, dark_mode=args.darkmode)
        else:
            print("Error: unexpected theme provided - ", args.theme)
...
def make_star_wars_movie(shot_list, dark_mode = False):
    ...
    if (dark_mode):
        dt_video = VideoFileClip("assets/speaking_video_clips/vader.mp4")
        dt_audio = "assets/speech_samples/vader.wav"
        dd_video = VideoFileClip("assets/speaking_video_clips/tarkin.mp4")
        dd_audio = "assets/speech_samples/tarkin.wav"
    else:
        dt_video = VideoFileClip("assets/speaking_video_clips/luke.mp4")
        dt_audio = "assets/speech_samples/luke.wav"
        dd_video = VideoFileClip("assets/speaking_video_clips/c3po.mp4")
        dd_audio = "assets/speech_samples/c3po.wav"
